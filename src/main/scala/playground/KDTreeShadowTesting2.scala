...
object KDTreeShadowTesting2 extends App {
  ...
  val size = 1e-5
  val spheres = Array.fill(2000)(GeomSphere(Point(math.random * size - size / 2, math.random * size, util.Random.nextGaussian()), 1e-7, p => RTColor.White, p => 0.0))
  val tree = new KDTreeGeometry(spheres, 1)
  val path = LinearViewPath(
    List(
      StopPoint(View(Point(0, -5e-6, 0), Vect(0, 10e-6, 0).normalize, Vect(0, 0, 10e-6).normalize), 10),
      StopPoint(View(Point(0, -5e-6, 10e-6), Vect(0, 20e-6, -10e-6).normalize, Vect(0, 10e-6, 20e-6).normalize), 10)),
    List(30), SmoothEasing)
  val lights = List(PointLight(RTColor.White, Point(0.0, -1 * math.cos(10 * math.Pi / 180), 1 * math.sin(10 * math.Pi / 180))))
