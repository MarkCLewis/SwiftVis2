...
//        println(hits)
  ...
  private def buildTree(geom: Seq[Geometry]): Node = {
    def helper(g: Seq[Geometry], min: Point, max: Point): Node = {
      ...
      if(children.isEmpty || g.length <= MaxGeom) {
        LeafNode(g, new BoundingSphere(actualMin+rad, rad.magnitude))
      } else {
        val splitDim = (max-min).maxDim
        val (splitValue, before, after) = partitionOnDim(g.toArray, splitDim)
        val left = helper(before, min, max.updateDim(splitDim, splitValue))
        val right = helper(after, min.updateDim(splitDim, splitValue), max)
        InternalNode(here, splitDim, splitValue, left, right, new BoundingSphere(actualMin+rad, rad.magnitude))
      }
    }
    helper(geom, geom.map(g => g.boundingSphere.center.offsetAll(-g.boundingSphere.radius)).reduceLeft(_ min _), 
        geom.map(g => g.boundingSphere.center.offsetAll(g.boundingSphere.radius)).reduceLeft(_ max _))
  }
